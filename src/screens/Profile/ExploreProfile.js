...
export default (props) => {
  ...
  async function getData() {
    let tmp = {};
    console.log(
      `other person from ExploreProfile.js : ${vidCntxt.viewProfile}`,
    );
    await firestore()
      .collection('user')
      .doc(vidCntxt.viewProfile)
      .get()
      .then((resp) => {
        tmp.followers = resp.data().followers;
        console.log(
          `followers from ExploreProfile : ${JSON.stringify(
            resp.data().followers,
          )}`,
        );
    ...
    <SafeAreaView style={{flex: 1}}>
      ...
      <View style={{flex: 0.05}}>
        <TouchableOpacity
          ...
          style={{marginTop: 10, alignSelf: 'flex-start', marginLeft: 10}}>
          ...
          <View style={{flex: 0.95}}>
            <View
              ...
              <TouchableOpacity
                style={[
                  ...
                  {
                    backgroundColor: usrCntxt.fllwingMap.get(
                      vidCntxt.viewProfile,
                    )
                      ? 'grey'
                      : '#a70624',
                  },
                ...
                onPress={() => {
                  ...
                  usrCntxt
                    .updateFollowing(
                      usrCntxt.fllwingMap.get(vidCntxt.viewProfile)
                        ? 'unfollow'
                        : 'follow',
                      vidCntxt.viewProfile,
                    )
            ...
            {userCont.myVideos != '' ? (
              <FlatList
                ...
                data={vidCntxt.videos.filter(
                  (item) => item.userid === vidCntxt.viewProfile,
                )}
                numColumns={3}
